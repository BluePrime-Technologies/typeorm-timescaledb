...
export function composeMigration(
  ...
  return {
    name: options.name ?? timescale.name,
    timestamp: options.timestamp ?? timescale.timestamp,
  ...
  const note =
    migration.filtered.length === 0
      ...
      : `\n// Statements removed because TimescaleDB owns their target (not lost — listed for review):\n${migration.filtered
          .map((f) => `//   [${f.side}] ${f.object} — ${f.reason}\n//     ${f.statement.sql}`)
          .join('\n')}\n`;
...
const NON_TRANSACTIONAL = /\bCONCURRENTLY\b|^\s*(?:VACUUM|CREATE\s+DATABASE|DROP\s+DATABASE)\b/i;
...
export function renderComposedMigrationSql(migration: ComposedMigration): string {
      ...
      if (NON_TRANSACTIONAL.test(sql)) {
        throw new TimescaleError(
          ...
          `Cannot emit .sql: a statement in the ${side} section cannot run inside a transaction ` +
            ...
            `back. Emit .ts instead, which is not wrapped: ${sql}`,
          { side, sql },
        );
      }
    }
  ...
  const note =
    migration.filtered.length === 0
      ? ''
      : `${migration.filtered
          .map((f) => `-- removed [${f.side}] ${f.object} — ${f.reason}\n--   ${f.statement.sql}`)
          .join('\n')}\n\n`;
