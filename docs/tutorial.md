...
Open the generated file. It should contain a TypeORM migration class similar to
this abbreviated example:

```ts
// Generated by typeorm-timescaledb — regenerate rather than editing by hand.
...
export class Timescale1700000000000 implements MigrationInterface {
  name = 'Timescale1700000000000';

  public async up(queryRunner: QueryRunner): Promise<void> {
    await queryRunner.query('SELECT create_hypertable(/* reading time */);');
    await queryRunner.query('ALTER TABLE /* enable columnstore */;');
    await queryRunner.query('CALL add_columnstore_policy(/* after 7 days */);');
    await queryRunner.query('SELECT add_retention_policy(/* 90 days */);');
  }

  public async down(queryRunner: QueryRunner): Promise<void> {
    await queryRunner.query('SELECT remove_retention_policy(/* reading */);');
    await queryRunner.query('CALL remove_columnstore_policy(/* reading */);');
  }
}
```

The exact timestamp, class name, and SQL arguments will differ. The important
thing is that the file is a TypeScript `MigrationInterface` containing the
TimescaleDB statements for the `reading` hypertable metadata.
