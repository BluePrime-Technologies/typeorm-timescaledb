...
**Not yet (planned):** continuous aggregates, the remaining toolkit aggregates (`heartbeat_agg` and `mcv_agg`; plus `gauge_agg`/`freq_agg`/`compact_state_agg` which are still `toolkit_experimental` upstream), the full diff engine, and validated cross-store references. **Unsupported by design:** automatic destructive/altering migrations.
