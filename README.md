...
**Not yet (planned):** continuous aggregates, the remaining toolkit aggregates (state / freq / heartbeat, plus `gauge_agg` which is still `toolkit_experimental` upstream), the full diff engine, and validated cross-store references. **Unsupported by design:** automatic destructive/altering migrations.
