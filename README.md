...
- **`timescaledb_toolkit` features** — `repo.getCandlesticks(...)` (typed OHLCV), `repo.approxCountDistinct(...)`, `repo.getStats(...)` / `repo.getRegression(...)` (statistics + linear regression), and `repo.getPercentiles(...)` / `repo.getPercentileRanks(...)` (approximate percentiles), all with a presence check that fails fast (`TSDB_TOOLKIT_MISSING`) when the extension isn't installed.
...
**Not yet (planned):** continuous aggregates, the remaining toolkit aggregates (counter / gauge / time-weight / state / freq / heartbeat), the full diff engine, and validated cross-store references. **Unsupported by design:** automatic destructive/altering migrations.
